...
public class RHF
{
    ...
    public int MaxCycle { get; set; } = 128;
    public double RequiredDelEnergy { get; set; } = 1e-12;
    private double RequiredMaxDelF { get; set; } = 1e-12;
    private double RequiredAverDelF { get; set; } = 1e-12;
    ...
    public class RHFDIIS(RHF rhf, int diisSpace = RHFDIIS.maxRHFDiis)
        : DIISBase(rhf.Ovlp.Rows * rhf.Ovlp.Rows, diisSpace)
    {
        ...
        public Matrix Invoke(Matrix fock, Matrix density)
        {
            ...
            inputs.SliceFirstDim(currentIndex, fock.AsNDArray());
            if (coeffs.Length <= 1)
                return fock;
