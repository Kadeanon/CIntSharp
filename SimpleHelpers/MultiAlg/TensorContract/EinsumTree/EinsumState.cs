...
namespace SimpleHelpers.MultiAlg.TensorContract.EinsumTree
{
    internal class EinsumState
    {
        ...
        public nint[] tensorSize;
        public NDArray?[] tensors;
        ...
        public EinsumState(string expression,
            params ReadOnlySpan<NDArray> tensors)
        {
            ...
            strides = new nint[inputCount, dimCount];
            dimLengths = new nint[dimCount];
            ...
            tensorSize = new nint[inputCount];
            for (int i = 0; i < inputCount; i++)
            {
                nint size = 0;
        ...
        private EinsumState(EinsumState parent, int left, int right, string name)
        {
            ...
            tensors = new NDArray[inputCount];
            nodes = parent.nodes;
            int i = 0;
            for (; i < left; i++)
            {
            ...
            out double cost, out string outputName)
        {
            ...
            nint leftSize = tensorSize[left];
            nint rightSize = tensorSize[right];
            nint resultSize = 1;
            nint flops = 1;
            for (int i = 0; i < dimCount; i++)
            {
